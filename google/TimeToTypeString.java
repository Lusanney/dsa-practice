 ...
 *
 * Given a string keyboard that describe the keyboard layout and a string text, return an
 * integer denoting the time taken to type string text.
 *
...
public class TimeToTypeString {
    private static int solve(String keyboardLayout, String text){
        HashMap<Character, Integer> keyboardIndex = new HashMap<>();

        for(int i=0; i < keyboardLayout.length(); i++){
            keyboardIndex.put(keyboardLayout.charAt(i), i);
        }

        int currentIdx = 0;
        int sum = 0;
        for(char c: text.toCharArray()){
            int idxOfC = keyboardIndex.get(c);
            sum += Math.abs(currentIdx - idxOfC);
            currentIdx = idxOfC;
        }
